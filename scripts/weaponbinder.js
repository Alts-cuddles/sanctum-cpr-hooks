...
function findTemplate(type) {
  ...
  return { src: list.find(e => e.primary?.sound?.file) || list[0] };
}
function cloneTemplate(type, name, customSound, exact) {
  const { src } = findTemplate(type);
    ...
    <form class="flexcol" style="gap:8px">
      ...
      <div><label><b>Apply to</b></label>
        <select id="aab-scope">
          ...
          <option value="global" selected>This item name (all players / world)</option>
  ...
  new Dialog({
    ...
    render: (html) => {
      ...
      r.querySelector("#aab-browse").addEventListener("click", ev => {
        ...
        new FilePicker({
          type: "audio",
          current: "assets/Weapon Sounds",
          callback: path => { r.querySelector("#aab-sound").value = cleanPath(path); }
        }).render(true);
      });
    }
...
function openPanel() {
  ...
  let relay = true;
  try { relay = game.settings.get(MODULE_ID, "aabBroadcastSounds"); } catch (e) {}
  const browse = game.permissions?.FILES_BROWSE?.includes(CONST.USER_ROLES.PLAYER);
  new Dialog({
    title: "Sanctum Weapon Binder",
    content: `<p>AA ${aa ? "on" : "OFF"} · Sequencer ${seq ? "on" : "OFF"} · Relay ${relay ? "on" : "off"} · Player browse ${browse ? "on" : "OFF"}</p>`,
    buttons: {
      binder: { icon: '<i class="fas fa-link"></i>', label: "Open Binder", callback: () => openBinder() },
      perms: {
      ...
      test: {
        ...
        callback: () => {
          ...
          if (!snd) return ui.notifications.warn("No bound sound on that token's first weapon.");
          playForRecipients(snd.file, snd.volume, game.users.filter(u => u.active).map(u => u.id));
        }
      }
    }
  }).render(true);
...
function attachApi() {
  ...
  globalThis.sanctumOpenBinder = openBinder;
  const mod = game.modules.get(MODULE_ID);
  if (mod) mod.api = Object.assign(mod.api || {}, { openPanel, openBinder, fixPlayback: enablePlayerPlayback });
...
Hooks.once("ready", () => {
  ...
  } catch (err) { console.warn("Sanctum Weapon Binder settings", err); }
  ...
  game.socket.on(SOCKET_EVENT, (payload) => {
    ...
    playForRecipients(
      payload.src,
      payload.volume,
      game.users.filter(u => u.active && (payload.includeGM || !u.isGM)).map(u => u.id)
    );
  });

  Hooks.on("AutomatedAnimations-WorkflowStart", (workflow) => {
    try { if (!game.settings.get(MODULE_ID, "aabBroadcastSounds")) return; } catch (e) { return; }
